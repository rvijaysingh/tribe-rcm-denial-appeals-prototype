...
6. `unsupported_required` is only for a required clause the chart says **nothing** about: the test was not done, the value is not recorded, the document is missing. Put it there with the specific document or result that would close the gap, stated concretely enough for a nurse to request it. A clause you list there must not appear in any `criteria_argument` assertion: conceding a gap and arguing the same clause is met contradicts itself, and the nurse reads both.

   Weak support is not a missing record. If the chart speaks to the clause but falls short of its threshold, covers only some of its elements, or rests on a single reading, argue it with the evidence that does exist, say precisely what the record shows, and let your confidence carry the weakness. Do not concede a clause merely because the evidence is thin: conceding sends the case back for records that are already in the chart, and the nurse loses the argument you could have made.
