...
7. Argue each criteria clause on its own terms, citing the lines that satisfy it. Where a clause has several elements, address each element.
   ...
   When citing a precedent, state only what the precedent's summary says it was denied for and what it was decided on. Do not infer or extend the precedent's basis to facts not in its summary, and do not restate its denial rationale as the one in front of you: a precedent denied for length of stay is not a precedent about documentation at admission. If the summary does not say, do not say it.
